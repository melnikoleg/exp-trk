...
const SentryTest: React.FC = () => {
  ...
  const testUnhandledError = () => {
    ...
    setTestResult(
      "Triggered unhandled error. Check Sentry dashboard in a moment.",
    );
  };
  ...
  const testPerformanceMonitoring = async () => {
    ...
    Sentry.startSpan({ name: "Test Transaction", op: "test" }, async () => {
      try {
        ...
        setTestResult(
          "Performance test completed! Check Sentry for the transaction.",
        );
      } catch (error) {
  ...
  return (
    <div className={styles.container}>
      ...
      <p className={styles.description}>
        Use these buttons to verify Sentry integration is working correctly. All
        actions will report to your Sentry dashboard.
      </p>
      ...
      <div className={styles.buttonGroup}>
        ...
        <Button onClick={testPerformanceMonitoring} disabled={isLoading}>
          {isLoading ? "Testing..." : "Test Performance Monitoring"}
        </Button>

        <Button onClick={testMessage} variant="primary" disabled={isLoading}>
          Test Message
        </Button>
      </div>
      ...
      <div className={styles.note}>
        <strong>Note:</strong> This component is for testing purposes only and
